...
if (!function_exists('caaft_zoho_exchange_grant_code')) {
    /**
     * Self Client setup — exchange a grant code from Zoho API Console (no redirect URI).
     *
     * @return array<string, mixed>|null
     */
    function caaft_zoho_exchange_grant_code(string $code): ?array
    {
        $config = caaft_zoho_config();
        ...
        return caaft_zoho_http_post($url, [
...
if (!function_exists('caaft_zoho_exchange_auth_code')) {
    /**
     * Server-based OAuth — exchange redirect callback code (requires matching redirect URI).
     *
     * @return array<string, mixed>|null
     */
    function caaft_zoho_exchange_auth_code(string $code): ?array
    {
        $config = caaft_zoho_config();
        $clientId = trim((string) ($config['client_id'] ?? ''));
        $clientSecret = trim((string) ($config['client_secret'] ?? ''));
...
if (!function_exists('caaft_zoho_save_refresh_token')) {
    function caaft_zoho_save_refresh_token(string $refreshToken, ?array $tokens = null): bool
    {
        ...
        $written = file_put_contents($localPath, "<?php\ndeclare(strict_types=1);\n\nreturn {$export};\n");
